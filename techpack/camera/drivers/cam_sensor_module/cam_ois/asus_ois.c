...
#define PAIR 3
	...
	pData = kzalloc(sizeof(uint32_t)*count*PAIR*2*2,GFP_KERNEL);
	if (!pData)
	{
		pr_err("no memory!\n");
		return -1;
	...
	for(i=0;i<count;i++)
	{
		for(j=0;j<PAIR;j++)
		{
			rc = onsemi_read_pair_sensor_data(ois_ctrl[ois_index],
											...
											pData+PAIR*2*2*i+2*2*j,//x
											pData+PAIR*2*2*i+2*2*j+1,//y
											pData+PAIR*2*2*i+2*2*j+2//timestamp
											);
			if(rc < 0)
			{
	...
	if(i == count)
	{
		...
		if(sysfs_write_dword_seq_change_line(RDATA_OUTPUT_FILE,pData,count*PAIR*2*2,PAIR*2*2,1) == 0)
		{
...
void update_OIS_gyro_gain_XY() {
	...
	if ( updated_OIS_gyro_gain==OIS_STATUS_INITIALIZE && g_ois_mode!=OIS_STATUS_INITIALIZE) {
		...
		if (g_ois_mode==OIS_STATUS_INITIALIZE)
			updated_OIS_gyro_gain=OIS_STATUS_INITIALIZE;
		return;
	}
	updated_OIS_gyro_gain=g_ois_mode;
	if (checkForNewBoard(Picasso_IMX686_Camera_ID)) {
