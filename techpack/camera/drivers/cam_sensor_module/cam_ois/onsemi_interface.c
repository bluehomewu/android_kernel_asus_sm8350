...
#include "FW/FromCode_05_05_01_p.h"
#else
								...
								)
{
	int rc;
        ...
        //uint64_t time = 0x00000002DFDC1C34;
        uint64_t time = 0x0;
	...
	//timestamp[0] = (time & 0xFFFF000000000000) >> 32;
	timestamp[0] = (time & 0x0000FFFF00000000) >> 16;
	timestamp[1] = (time & 0x00000000FFFF0000);
	...
	memset(i2c_write_setting_gl, 0, sizeof(struct cam_sensor_i2c_reg_array)*MAX_DATA_NUM*2);
        /*while(value>350000){*/
	        ...
	        if (rc < 0) {
                ...
                //if(j>0)
                        //CAM_ERR(CAM_OIS,"write time=%d",value);
               // j++;
        //}
        return rc;
}
...
int OIS_READ_HALL_DATA_TO_UMD_NEW (struct cam_ois_ctrl_t *o_ctrl,struct i2c_settings_array *i2c_settings) {
        #define SAMPLE_COUNT_IN_NCS_DATA 255
        int32_t                         rc = 0,i=0;
        ...
        list_for_each_entry(i2c_list,
                        ...
                        //OISCountinueRead(o_ctrl, 0xF111, (void *)read_buff, read_length);
                        rc = onsemi_read_seq_bytes(o_ctrl, 0xF111, read_buff, read_length);
                        ...
                        if((newqtime-(fifo_count-1)*2000000)<preqtime_ms)
                                CAM_ERR(CAM_OIS,"jayjay  time error");
                        ...
                        if(fifo_count > 0) {
                                for(i=0 ; i< SAMPLE_COUNT_IN_NCS_DATA ; i++){
                                        ...
                                        //CAM_ERR(CAM_OIS,"hall_data_x=0x%x hall_data_y=0x%x ",hall_data_x,hall_data_y);
                                }
                        }
        }
        return rc;
