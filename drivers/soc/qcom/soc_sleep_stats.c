...
#ifdef CONFIG_MACH_ASUS
#if defined ASUS_SAKE_PROJECT || defined ASUS_VODKA_PROJECT || defined ASUS_PICASSO_PROJECT
int pre_aosd_count;
...
void soc_sleep_stats_print(void)
{
	...
	for (i = 0; i < drv->config->num_records; i++) {
        ...
        length += scnprintf(buf + length, PAGE_SIZE - length, 
             ...
             ae->client_votes);                           
#if 0        
        printk("[PM]{%s  Count:%u  Entered:%llu(s)  Exited:%llu(s) Duration:%llu(s)  Client Votes:0x%x}",
             stat_type, e->count,
             e->last_entered_at,
             e->last_exited_at,
             e->accumulated,
