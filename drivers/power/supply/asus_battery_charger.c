...
struct wake_lock cos_boot_wake_lock;
//[---] Add the global variables
...
bool boot_completed_flag = 0; //ASUS_BSP LiJen add boot_completed attr
...
//ASUS_BSP +++ LiJen add boot_completed attr
					...
					const char *buf, size_t count)
{
	...
	CHG_DBG("%s: boot_completed_flag : %d\n", __func__, boot_completed_flag);
	return count;
}
...
static struct attribute *asuslib_class_attrs[] = {
	...
	&class_attr_boot_completed.attr, //ASUS_BSP LiJen add boot_completed attr
	NULL,
};
...
int asus_set_vbus_attached_status(int value) {
	...
	if (value) {
		...
		//wake_unlock(&cos_boot_wake_lock);
	}
	CHG_DBG("%s: g_vbus_plug : %d \n", __func__, g_vbus_plug);
	return 0;
}
...
void set_qc_stat(int status)
{
	...
	default:
		cancel_delayed_work(&asus_set_qc_state_work);
		if (g_ADAPTER_ID || rt_chg_check_asus_vid()) {
			CHG_DBG("%s: VID done but still Discharging, wait for Charging\n", __func__);
		} else {
			asus_extcon_set_state_sync(quickchg_extcon, SWITCH_LEVEL0_DEFAULT);
			CHG_DBG("%s: status: %d, switch: %d\n", __func__, status, SWITCH_LEVEL0_DEFAULT);
		}
...
int asuslib_init(void) {
	...
	if (!gpio_is_valid(PMI_MUX_GPIO)) {
		pr_err("%s: PMI_MUX_EN gpio is invalid %d\n", __func__, PMI_MUX_GPIO);
		return -1;
	} else {
	...
	if (!gpio_is_valid(POGO_OTG_GPIO)) {
		pr_err("%s: POGO_OTG_EN gpio is invalid %d\n", __func__, POGO_OTG_GPIO);
		return -1;
	} else {
	...
	side_usb_temp_vadc_chan = iio_channel_get(g_bcdev->dev, "pm8350b_amux_thm4");
	if (IS_ERR_OR_NULL(side_usb_temp_vadc_chan)) {
	...
	wake_lock_init(&cos_boot_wake_lock, g_bcdev->dev, "cable_resume_wake_lock");
	...
	if (g_Charger_mode){
		...
		wake_lock_timeout(&cos_boot_wake_lock, msecs_to_jiffies(90000));
	}
	else
...
int asuslib_deinit(void) {
	...
	wake_lock_destroy(&cos_boot_wake_lock);
