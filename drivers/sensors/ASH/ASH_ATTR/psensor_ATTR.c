...
#ifdef CONFIG_TMD2755_FLAG
static ssize_t  ATT_proximity_store_chip_offset(struct device *dev, 
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long reenable;	
	...
	if ((kstrtoul(buf, 10, &reenable) < 0))
		return -EINVAL;

	g_psensor_ATTR->ATTR_Extension->proximity_chip_recal();
	log("Proximity store chip cal: %lu\n", reenable);
...
#ifdef CONFIG_TMD2755_FLAG
	__ATTR(chip_recal, 0664, NULL, ATT_proximity_store_chip_offset),
#endif
};
