...
static ssize_t mproximity_chip_recal(void){
	log("cal mproximity_chip_recal");
	g_ALSPS_hw_client->mpsensor_hw->proximity_hw_chip_cal();
	return true;
}
#else
...
#ifdef CONFIG_TMD2755_FLAG
	.proximity_chip_recal = mproximity_chip_recal,
#endif
};
...
#ifdef CONFIG_TMD2755_FLAG
			if(g_ps_data->g_ps_int_status != state){
