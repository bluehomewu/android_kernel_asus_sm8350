...
static int sx932x_Hardware_Check(psx93XX_t this)
{
	...
	cap2_info("sx932x failcode = 0x%x\n",this->failStatusCode);
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x,%x", &reg_address, &val) != 2) {
		cap2_err("[SX932x]: %s - The number of data are wrong\n",__func__);
		return -EINVAL;
	}

	write_register(this, (unsigned char)reg_address, (unsigned char)val);
	//cap2_info("[SX932x]: %s - Register(0x%x) data(0x%x)\n",__func__, reg_address, val);
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	//cap2_info("[SX932x]: %s - Register(0x%2x) data(0x%2x)\n",__func__, regist, val);
...
#if 1
static int read_cap2_status0(psx93XX_t this)
{
	int ret;
...
static ssize_t sx932x_cap2_status0_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
    psx93XX_t this = dev_get_drvdata(dev);
	int ret = read_cap2_status0(this);
	if (ret == 0) {
...
static DEVICE_ATTR(cap2_status0,0664,sx932x_cap2_status0_show,NULL);
...
static int initialize(psx93XX_t this)
{
	...
	if (this) {
		...
		enable_irq(this->irq);
		ret = pinctrl_select_state(sx932x_pinctrl, sx932x_pinctrl_state_active);
		if(ret < 0)
			cap2_err("%s: failed to set sx932x pinctrl active state", __func__);
		/* make sure no interrupts are pending since enabling irq will only
		* work on next falling edge */
		read_regStat(this);
		cap2_info("SX932x initialize finished\n");
		return 0;
	}
	return -ENOMEM;
}
...
static void cap_status_uevent_far(psx93XX_t this)
{
	...
	//cap_info("%s start",__func__);
	if(g_CAP_STATUS_UEVENT_last != g_CAP_STATUS_UEVENT)
		read_cap2_status0(this);
}
...
static int sx932x_parse_dt(struct sx932x_platform_data *pdata, struct device *dev)
{
	...
	if (pdata->irq_gpio < 0) {
		cap2_err("[SENSOR]: %s - get irq_gpio error\n", __func__);
		return -ENODEV;
	}
	/***********************************************************************/
	...
	//cap2_info("[SX932x]:%s -  size of elements %d \n", __func__,pdata->i2c_reg_num);
	...
	/***********************************************************************/
	//cap2_info("[SX932x]: %s -[%d] parse_dt complete\n", __func__,pdata->irq_gpio);
	return 0;
}
...
static int sx932x_init_platform_hw(struct i2c_client *client)
{
	...
	//cap2_info("[SX932x] : %s init_platform_hw start!",__func__);
	...
	if (this && (pDevice = this->pDevice) && (pdata = pDevice->hw)) {
		...
		cap2_err("[SX932x] : %s - Do not init platform HW", __func__);
	}
	
	cap2_err("[SX932x]: %s - sx932x_irq_debug\n",__func__);
	return rc;
}
...
//ASUS_BSP  initial work for resume
static void sx93XX_2nd_init_worker_func(struct work_struct *work)
{
	psx93XX_t this = 0;
	if (work) {
		this = container_of(work,sx93XX_t,initworker2.work);
...
{    
	...
	if(g_Charger_mode){
		cap2_info("%s COS didn't start",__func__);
		return 0;
	}

	cap2_info("%s start",__func__);
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_READ_WORD_DATA)) {
		cap2_err("Check i2c functionality.Fail!\n");
		err = -EIO;
		return err;
	}
	...
	if (!pplatData) {
		cap2_err("platform data is required!\n");
		return -EINVAL;
	}
...
static void sx93XX_worker_func(struct work_struct *work)
{
	...
	if (work) {
		this = container_of(work,sx93XX_t,dworker2.work);
		...
		while((++counter) < MAX_NUM_STATUS_BITS) { /* counter start from MSB */
			if (((status>>counter) & 0x01) && (this->statusFunc[counter])) {
				//cap2_info("SX932x Function Pointer Found. Calling\n");
		...
		{	/* Early models and if RATE=0 for newer models require a penup timer */
			...
			cap2_info("sx93XX_worker_func, Early models");
...
{
	if (this) {
		cancel_delayed_work_sync(&this->dworker2); /* Cancel the Worker Func */
...
void sx93XX_2nd_suspend(psx93XX_t this)
{
	...
	rc = pinctrl_select_state(sx932x_pinctrl, sx932x_pinctrl_state_suspend);
	if(rc)
...
{	
	...
	if (this) {
		...
		//if (this->init)
			//this->init(this);
		schedule_delayed_work(&this->initworker2,0);
		rc = pinctrl_select_state(sx932x_pinctrl, sx932x_pinctrl_state_active);
		if(rc)
		cap_err("%s: failed to set sx932x pinctrl state", __func__);
...
int sx93XX_2nd_IRQ_init(psx93XX_t this)
{
	...
	if (this && this->pDevice)
	{
		...
		INIT_DELAYED_WORK(&this->dworker2, sx93XX_worker_func);
		INIT_DELAYED_WORK(&this->initworker2, sx93XX_2nd_init_worker_func); // add for reduce resume time
		...
		//ASUS pin ctrl +++
		//cap2_info(this->pdev, "registered with irq (%d)\n", this->irq);
		// balanced irq call
		disable_irq_nosync(this->irq);
