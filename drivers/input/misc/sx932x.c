			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	//cap_info("[SX932x]: %s - Register(0x%2x) data(0x%2x)\n",__func__, regist, val);
...
#if 1
static int read_cap_status0(psx93XX_t this)
{
	int ret;
...
static ssize_t sx932x_cap_status0_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
    psx93XX_t this = dev_get_drvdata(dev);
	int ret = read_cap_status0(this);
	if (ret == 0) {
...
static DEVICE_ATTR(cap_status0,0664,sx932x_cap_status0_show,NULL);
...
static int initialize(psx93XX_t this)
{
	...
	if (this) {
		...
		* work on next falling edge */
		read_regStat(this);
		cap_info("SX932x initialize finished\n");
		return 0;
	}
	return -ENOMEM;
}
...
static int sx932x_parse_dt(struct sx932x_platform_data *pdata, struct device *dev)
{
	...
	if (pdata->irq_gpio < 0) {
		cap_err("[SENSOR]: %s - get irq_gpio error\n", __func__);
		return -ENODEV;
	}
	/***********************************************************************/
	...
	//cap_info("[SX932x]:%s -  size of elements %d \n", __func__,pdata->i2c_reg_num);
	...
	/***********************************************************************/
	//cap_info("[SX932x]: %s -[%d] parse_dt complete\n", __func__,pdata->irq_gpio);
	return 0;
}
...
static int sx932x_init_platform_hw(struct i2c_client *client)
{
	...
	//cap_info("[SX932x] : %s init_platform_hw start!",__func__);
	...
	if (this && (pDevice = this->pDevice) && (pdata = pDevice->hw)) {
		...
		cap_err("[SX932x] : %s - Do not init platform HW", __func__);
	}
	
	cap_err("[SX932x]: %s - sx932x_irq_debug\n",__func__);
	return rc;
}
...
static void sx93XX_init_worker_func(struct work_struct *work)
{
	psx93XX_t this = 0;
	if (work) {
		this = container_of(work,sx93XX_t,initworker1.work);
...
{    
	...
	if(g_Charger_mode){
		cap_info("%s COS didn't start",__func__);
		return 0;
	}
	...
	//cap_info("SX932x init_platform_hw done!\n");
...
static void sx93XX_worker_func(struct work_struct *work)
{
	...
	if (work) {
		this = container_of(work,sx93XX_t,dworker1.work);
		...
		while((++counter) < MAX_NUM_STATUS_BITS) { /* counter start from MSB */
			if (((status>>counter) & 0x01) && (this->statusFunc[counter])) {
				...
				//cap_info("SX932x Function Pointer Found. Calling\n");
		...
		{	/* Early models and if RATE=0 for newer models require a penup timer */
			...
			cap_info("sx93XX_worker_func, Early models");
...
{
	if (this) {
		cancel_delayed_work_sync(&this->dworker1); /* Cancel the Worker Func */
...
{	
	...
	if (this) {
		...
		//if (this->init)
			//this->init(this);
		schedule_delayed_work(&this->initworker1,0);
...
int sx93XX_IRQ_init(psx93XX_t this)
{
	...
	if (this && this->pDevice)
	{
		...
		INIT_DELAYED_WORK(&this->dworker1, sx93XX_worker_func);
		INIT_DELAYED_WORK(&this->initworker1, sx93XX_init_worker_func); // add for reduce resume time
		...
		//ASUS pin ctrl +++
		//cap_info(this->pdev, "registered with irq (%d)\n", this->irq);
		// balanced irq call
		disable_irq_nosync(this->irq);
