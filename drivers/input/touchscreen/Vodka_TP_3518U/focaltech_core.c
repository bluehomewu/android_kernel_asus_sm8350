...
// fod
static int fts_fp_position[4] = {415, 665, 1681, 1931};
...
//        FTS_INFO("DRM_PANEL_BLANK_FPS_CHANGE, refresh rate %d", evdata->refresh_rate);
        break;
    default:
...
int fts_ts_suspend(struct device *dev)
{
    ...
    if (fts_init_success == 0) {
        FTS_INFO("Driver does not init successful, skip suspend.");
    ...
    if (enter_gesture_mode == 1) {
            ...
            if (gesture_suspend_ok > 0) {
                FTS_INFO("Enter gesture mode successfully");
            }
    } 

    if ((enter_gesture_mode != 1) || (gesture_suspend_ok < 0)) {
...
static int fts_ts_resume(struct device *dev)
{
    ...
    if (fts_init_success == 0) {
        FTS_INFO("Driver does not init successful, skip resume.");
        mutex_unlock(&fts_data->suspend_mutex);
        return 0;
    }
...
void ftxxxx_disable_touch(bool flag)
{
    if (flag)
    {
        ...
        fts_release_all_finger();
        FTS_INFO("%s: proximity trigger disable touch !\n", __func__);
    }
    else
    {
        disable_tp_flag = false;
        FTS_INFO("%s: proximity trigger enable touch  !\n", __func__);
    }
...
static int fts_resume(struct device *dev)
{
	...
	if (fts_init_success==1){
		...
		FTS_INFO("%s: focal touch not work skip resume \n", __func__);
	return 0;
}
*/
// Vodka_BSP ---
